...
test_single_ptr_arg_ptr_ret(void)
{
	...
	u = ptr_lookup("tok-bob");
	assert(u != NULL);
	assert(strcmp(u, "bob") == 0);

	u = ptr_lookup("tok-nonexistent");
	assert(u == NULL);
...
test_repeated_calls_same_hook(void)
{
	for (int i = 0; i < 1000; i++) {
		const char *u = ptr_lookup("tok-carol");
		assert(u != NULL);
...
test_multi_ptr_args(void)
{
	...
	int r = ptr_copy(buf, "hello", sizeof(buf));
	assert(r == 5);
...
{
	const char *u = ptr_resolve_remote(1);
	assert(u != NULL);
	assert(strcmp(u, "alice") == 0);

	u = ptr_resolve_remote(2);
	assert(u != NULL);
	assert(strcmp(u, "bob") == 0);

	/* Hammer it. */
	for (int i = 0; i < 500; i++) {
		u = ptr_resolve_remote(((i % 3) + 1));
		assert(u != NULL);
	}
