# Releasing the xy Rust crates

The two crates (`xy-macros`, `xy`) are always released together at the same
...
in `xy/Cargo.toml`'s `[dependencies]`:

```sh
# Edit these two files:
rust/xy-macros/Cargo.toml
rust/xy/Cargo.toml        # also update version = "X.Y" in the dep entry
```
...
```sh
cargo publish --dry-run --manifest-path rust/xy-macros/Cargo.toml
cargo publish --dry-run --manifest-path rust/xy/Cargo.toml
```
...
```sh
cargo publish --manifest-path rust/xy-macros/Cargo.toml
cargo publish --manifest-path rust/xy/Cargo.toml
```
...
- If `libxylem` itself is being released at the same time, publish the C library
